...
public class SPQPNode extends SPQNode {
    ...
    public void computeOrthogonalRepresentation(HashMap<MutablePair<TreeVertex, TreeVertex>, Integer> hashMap) {
        ...
        if (startNodes.size() == 3 && !this.isRoot) {
            ...
            hashMap.put((new TupleEdge<>(nextVertexStarRight, startVertex)), 1);
            ...
            hashMap.put((new TupleEdge<>(startNodes.get(1), startVertex)), 1);

            // Beispiel3-4-10  Außen
            hashMap.put((new TupleEdge<>(startNodes.get(2), startVertex)), 1);

            // Beispiel5-4-3  Außen
            hashMap.put((new TupleEdge<>(startNodes.get(0), startVertex)), 1);
            ...
            hashMap.put(new TupleEdge<>(vertex1, startVertex), alphaul);
            ...
            hashMap.put((new TupleEdge<>(nextVertexStarRight, startVertex)), alphaur);

            //Winkel zwischen der linken und rechten äußeren Kanten "innen" (Bsp. am Ende von Kante 7-6 an Knoten 6)
            hashMap.put((new TupleEdge<>(vertex2, startVertex)), ((alphaur + alphaul == 2) && (startVertex.adjecentVertices.size() == 3)) ? 0 : 1);

        } else if (startVertex.adjecentVertices.size() == 2 && this.isRoot) {

            hashMap.put((new TupleEdge<>(sinkVertex, startVertex)), 1);
            hashMap.put((new TupleEdge<>(startNodes.get(0), startVertex)), -1);
        ...
        if (sinkNodes.size() == 3 && !this.isRoot) {
            ...
            hashMap.put((new TupleEdge<>(nextVertexSinkLeft, sinkVertex)), 1);

            // 8-13-7
            hashMap.put((new TupleEdge<>(sinkNodes.get(0), sinkVertex)), 1);

            // Beispie 7-13-12  "Zwischen Innenkanten"
            hashMap.put((new TupleEdge<>(sinkNodes.get(1), sinkVertex)), 1);

            // Beispie 12-13-14  "Zwischen Innenkanten"
            hashMap.put((new TupleEdge<>(sinkNodes.get(2), sinkVertex)), 1);
            ...
            hashMap.put((new TupleEdge<>(nextVertexSinkLeft, sinkVertex)), alphavl);
            ...
            hashMap.put((new TupleEdge<>((mergedChildren.get(1).sinkNodes.get(0)), sinkVertex)), alphavr);

            //Winkel zwischen der linken und rechten äußeren Kanten "innen" (Bsp. am Ende von Kante 9-11-10 an Knoten 11)
            hashMap.put((new TupleEdge<>(sinkNodes.get(0), sinkVertex)), (alphavr + alphavl == 2 && (sinkVertex.adjecentVertices.size() == 3)) ? 0 : 1);

        } else if (sinkVertex.adjecentVertices.size() == 2 && this.isRoot) {

            hashMap.put((new TupleEdge<>(startVertex, sinkVertex)), -1);
            hashMap.put((new TupleEdge<>(sinkNodes.get(0), sinkVertex)), 1);
